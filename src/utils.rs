...
pub fn add_extra_type_param_bound_op_output<'a>(
    ...
    for type_param in &mut generics.type_params_mut() {
        ...
        let bound: TypeParamBound =
            parse_str(&quote!(#import_root::ops::#trait_ident<Output=#type_ident>).to_string())
                .unwrap();
...
pub fn add_extra_ty_param_bound_op<'a>(
    ...
    add_extra_ty_param_bound(generics, &quote!(#import_root::ops::#trait_ident))
}
...
pub fn add_where_clauses_for_new_ident<'a>(
    ...
    let generic_param = if fields.len() > 1 {
        quote!(#type_ident: #import_root::marker::Copy)
    } else {
        quote!(#type_ident)
    };
