...
use utils::{field_idents, get_field_types, numbered_vars};
...
pub fn expand(input: &DeriveInput, _: &str) -> Tokens {
    ...
    let ((body, vars), fields) = match input.body {
        Data::Struct(ref data_struct) => match data_struct.fields {
            Fields::Unnamed(ref fields) => (tuple_body(input_type, fields), fields),
            Fields::Named(ref fields) => (struct_body(input_type, fields), fields),
            Fields::Unit => (struct_body(input_type, empty_fields), empty_fields),
        },
