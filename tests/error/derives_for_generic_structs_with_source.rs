...
const fn works_with_various_generics_combinations() {
    ...
    #[derive(Debug, Display, Error)]
    struct ErrConst<const X: usize> {}

    #[derive(Debug, Display, Error)]
    struct ErrLifetime<'lt: 'static> {
        inner: &'lt Err,
    }

    #[derive(Debug, Display, Error)]
    struct ErrConstGeneric<const X: usize, E> {
        inner: E,
    }

    #[derive(Debug, Display, Error)]
    struct ErrGenericConst<E, const X: usize> {
        inner: E,
    }
    #[derive(Debug, Display, Error)]
    struct ErrGenericConstDefault<E, const X: usize = 0> {
        inner: E,
    }
    #[derive(Debug, Display, Error)]
    struct ErrLifetimeGeneric<'lt: 'static, E> {
        inner: &'lt E,
    }

    #[derive(Debug, Display, Error)]
    struct ErrLifetimeConst<'lt: 'static, const X: usize> {
        inner: &'lt ErrConst<X>,
    }

    #[derive(Debug, Display, Error)]
    struct ErrLifetimeConstGeneric<'lt: 'static, const X: usize, E> {
        inner: &'lt E,
    }
    #[derive(Debug, Display, Error)]
    struct ErrLifetimeGenericConst<'lt: 'static, E, const X: usize> {
        inner: &'lt E,
    }
}
