...
This does not support custom error with generic parameters. For example, `struct CustomError<T>(TryIntoError<T>)`, a custom error that is a newtype wrapper around `TryIntoError`, is not supported.
